...
const importConfigs: ImportConfig[] = [
  {
    type: "sales",
    title: "Sales Report",
    description: "MobiPOS monthly closing — maps Total Sales, Category, Discount & Bill Discount to calculate Net Revenue.",
    icon: DollarSign,
    accept: ".csv",
  ...
  },
  {
    ...
    description: "ABA bank statement — compares Transaction Details & Money In against sales. Extracts Bill IDs from Remark text.",
    icon: Landmark,
    accept: ".csv",
    iconClass: "text-info",
    bgClass: "bg-info/10",
  },
  {
    ...
    description: "Physical inventory — calculates Lost stock (Physical QTY − System QTY) and dollar value using COG.",
    icon: Warehouse,
    accept: ".csv",
      ...
      try {
        ...
        switch (type) {
          case "sales": {
            ...
            message = `${recordCount} sales records. Net Revenue: $${totalRevenue.toLocaleString()}. Total Discounts: $${totalDiscount.toLocaleString()}.`;
            break;
          }
          case "bank": {
            const records = parseBankStatementCSV(text);
            recordCount = records.length;
            const matched = records.filter((r) => r.matched_bill_id).length;
            const totalIn = records.reduce((s, r) => s + r.money_in, 0);
            message = `${recordCount} transactions. ${matched} auto-matched to bills. Total Money In: $${totalIn.toLocaleString()}.`;
            break;
          }
          case "inventory": {
            const records = parseInventoryCSV(text);
            recordCount = records.length;
            const totalLoss = records.reduce((s, r) => s + r.loss_value, 0);
            const lostItems = records.filter((r) => r.lost_qty !== 0).length;
            message = `${recordCount} items tracked. ${lostItems} with stock discrepancy. Total loss value: $${totalLoss.toLocaleString()}.`;
            break;
          }
          case "income": {
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {importConfigs.map((cfg) => {
          ...
          return (
            <div
              key={cfg.type}
              className="glass-card p-5 flex flex-col gap-4 transition-all hover:glow-primary"
            >
